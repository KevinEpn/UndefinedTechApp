...
export class Level3Component {
  ...
  alerts: { [key: number]: { type: 'info' | 'warning' | 'success'} | null } = {};
  ...
  handleCardClick(cardId: number, type: 'info' | 'warning' | 'success') {
    if (this.alerts[cardId]) {
      delete this.alerts[cardId];
    }

    if (type === 'success') {
      this.correctCards.add(cardId);
      this.alerts[cardId] = { type: 'success' };      
  ...
  checkCompletion() {
    // Check if cards 2, 4 (correct ones) are selected
    if (this.correctCards.has(2) && this.correctCards.has(4) && !this.correctCards.has(1) && !this.correctCards.has(3)) {
  ...
  showModalCompleted(): void {
    ...
    const tiempoEnMinutos = this.elapsedTime / 60; // Convertir s a minutos
  ...
  onKeyPress(event: KeyboardEvent, id: number, type: string): void {
    ...
    if (event.key === 'Enter' || event.key === ' ') {
      ...
      this.handleCardClick(id, type as 'info' | 'warning' | 'success');
    }
  }
